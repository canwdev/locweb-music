const {getExactPath, getMediaPath, getSafeFilename} = require('../../utils/fs-tool')
...
const downloadFile = async (req, res, next) => {
  try {
    const {filename, path: musicPath = '', saveName} = req.query
...
const uploadFile = async (req, res, next) => {
  try {
    ...
    let {filename, path: musicPath = ''} = req.body
