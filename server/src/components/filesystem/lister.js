const {getExactPath, getMediaPath, getLyricsPath} = require('../../utils/fs-tool')
const {lyricFileCache} = require('./cache')
const {enableAuth, showHiddenFiles} = require('../../config')
const fs = require('fs-extra')
const path = require('path')
const {loginOnlyFileName} = require('../../config/enum')
const mfpTool = require('../../utils/mfp-tool')
const {getSafePath} = require('../../utils/fs-tool')

const {getMetadata, traverseLyrics, filterLyricFilename} = require('../../utils/music-tool')
...
const listFiles = async (req, res, next) => {
  try {
    const {path: musicPath = '', getPlayStat = false} = req.query
...
const getDetail = async (req, res, next) => {
  try {
    const {
      path: musicPath = '',
      filename,
      updatePlayStat = false, // 是否更新文件夹播放状态
      updateStatOnly = false, // 只更新状态，不获取元数据信息
    } = req.query
